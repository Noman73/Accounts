...
class SupplierController extends Controller
{
    ...
    public function ManageSupplier(){
    	if (request()->ajax()) {
            $total_bal=500;
            $get=DB::select("select id,name,phone,adress,supplier_type from suppliers");
    ...
    public function searchSupplier(Request $r){
         if (!preg_match("/[^a-zA-Z0-9 ]/", $r->searchTerm)) {
            $data=DB::select("SELECT id,name,phone from suppliers where name like '%".$r->searchTerm."%' or  phone like '%".$r->searchTerm."%' limit 10");
