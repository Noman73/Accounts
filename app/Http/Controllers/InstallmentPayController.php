...
class InstallmentPayController extends Controller
{
    ...
    public function getInsInvoice(Request $r){
    	if (!preg_match("/[^a-zA-Z0-9. ]/", $r->searchTerm)) {
            $data=DB::select("SELECT invoices.id,customers.id customer_id,customers.name,cast(invoices.total_payable-((invoices.total_payable*ifnull(invoices.insmnt_pay_percent,0))/100) as decimal(20,2)) as total_payable from invoices
            inner join customers on customers.id=invoices.customer_id and invoices.action_id=3
            where invoices.action_id=3  and customers.name like '%".$r->searchTerm."%' or invoices.id like '%".$r->searchTerm."%' or customers.id like '%".$r->searchTerm."%' limit 10");
